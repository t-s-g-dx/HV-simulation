...
function dzdt = odefcn(t,z,tx,x,lam)
  ...
  dzdt(nz-5) = v1;
  dzdt(nz-4)=-lam(nz-4)*sign(z(nz-5)-x(nx-2));
  v2=-lam(nz-3)*abs(z(nz-3)-x(nx-1)).^(1/2)*sign(z(nz-3)-x(nx-1))+z(nz-2);
  dzdt(nz-3) = v2;
  dzdt(nz-2)=-lam(nz-2)*sign(z(nz-3)-x(nx-1));
  v3=-lam(nz-1)*abs(z(nz-1)-x(nx)).^(1/2)*sign(z(nz-1)-x(nx))+z(nz);
  dzdt(nz-1)=v3;
  dzdt(nz)=-lam(nz)*sign(z(nz-1)-x(nx));
  i=1;
  j=1;
  while i<(nz-6)&&j<(nx-5)
  v1=-lam(i)*abs(z(i)-(x(j)-x(nx-5))).^(2/3)*sign(z(i)-(x(j)-x(nx-5)))+z(i+1);
  dzdt(i) = v1;
  v2=-lam(i+1)*abs(z(i)-(x(j)-x(nx-5))).^(1/3)*sign(z(i)-(x(j)-x(nx-5)))+z(i+2);
  dzdt(i+1) = v2;
  dzdt(i+2)=-lam(i+2)*sign(z(i)-(x(j)-x(nx-5)));

  v3=-lam(i+3)*abs(z(i+3)-(x(j+1)-x(nx-4))).^(2/3)*sign(z(i+3)-(x(j+1)-x(nx-4)))+z(i+4);
  dzdt(i+3) = v3;
  v4=-lam(i+4)*abs(z(i+3)-(x(j+1)-x(nx-4))).^(1/3)*sign(z(i+3)-(x(j+1)-x(nx-4)))+z(i+5);
  dzdt(i+4) = v4;
  dzdt(i+5)=-lam(i+5)*sign(z(i+3)-(x(j+1)-x(nx-4)));

  v5=-lam(i+6)*abs(z(i+6)-(x(j+2)-x(nx-3))).^(2/3)*sign(z(i+6)-(x(j+2)-x(nx-3)))+z(i+7);
  dzdt(i+6) = v5;
  v6=-lam(i+7)*abs(z(i+6)-(x(j+2)-x(nx-3))).^(1/3)*sign(z(i+6)-(x(j+2)-x(nx-3)))+z(i+8);
  dzdt(i+7) = v6;
  dzdt(i+8)=-lam(i+8)*sign(z(i+6)-(x(j+2)-x(nx-3)));
  i=i+9;
  j=j+6;
  end
end