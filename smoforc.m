...
function dzdt = odefcn(t,z,tx,x,lam)
  ...
  dzdt(1) = v1;
  v2=-lam(2)*abs(z(2)-v1).^(1/2)*sign(z(2)-v1)+z(3);
  dzdt(2) = v2;
  dzdt(3)=-lam(3)*sign(z(3)-v2);

  v3=-lam(4)*abs(z(4)-x(2)).^(2/3)*sign(z(4)-x(2))+z(5);
  dzdt(4) = v3;
  v4=-lam(5)*abs(z(5)-v3).^(1/2)*sign(z(5)-v3)+z(6);
  dzdt(5) = v4;
  dzdt(6)=-lam(6)*sign(z(6)-v4);

  v5=-lam(7)*abs(z(7)-x(3)).^(2/3)*sign(z(7)-x(3))+z(8);
  dzdt(7) = v5;
  v6=-lam(8)*abs(z(8)-v5).^(1/2)*sign(z(8)-v5)+z(9);
  dzdt(8) = v6;
  dzdt(9)=-lam(9)*sign(z(9)-v6);
